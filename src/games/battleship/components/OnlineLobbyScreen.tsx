...
import { GAME_MODES, MAX_PLAYERS, MIN_PLAYERS } from '@/games/battleship/constants/board';
...
export function OnlineLobbyScreen() {
  ...
  const beginMatch = useOnlineStore((s) => s.beginMatch);
  ...
  const create = () => {
    ...
    setOnlineName(clean);
    void hostRoom(clean || t('player'), { mode: setupMode });
  };
  ...
  useEffect(() => {
    if (status === 'match') router.replace('/battleship/game');
  }, [status, router]);
      ...
      <SafeAreaView style={[styles.screen, { backgroundColor: theme.background }]}>
        ...
        {isHost ? (
          <View style={styles.config}>
            ...
            <Segmented<GameMode>
              options={GAME_MODES.map((value) => ({
                value,
                label: t(value === 'classic' ? 'mode_classic' : 'mode_moving'),
              }))}
              value={lobby.config.mode}
              onChange={(next) => setConfig({ mode: next })}
              hint={t(lobby.config.mode === 'classic' ? 'mode_classic_hint' : 'mode_moving_hint')}
            />
          </View>
        ) : null}
