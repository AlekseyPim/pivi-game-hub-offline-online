...
import { MAX_PLAYERS } from '@/games/sudoku/constants/board';
import { createGame } from '@/games/sudoku/logic/gameReducer';
import type {
  Intent,
  LobbySnapshot,
  MatchConfig,
  NetMessage,
  Seat,
} from '@/games/sudoku/net/protocol';
import { reduceIntent } from '@/games/sudoku/net/session';
...
import type { GameState, PlayerConfig } from '@/games/sudoku/types/game';

/**
 * Orchestrates online, host-authoritative multiplayer over a Supabase broadcast
 * channel (`room:sd:{code}`) — the same architecture as ludo-game.
 *
 * Sudoku hides nothing, so unlike battleship the host really can own the whole
 * state: it generates the puzzle, carves the two territories and broadcasts
 * every snapshot.
 *
...
const DEFAULT_CONFIG: MatchConfig = { difficulty: 'easy' };

/** Persisted per-room player id so a returning guest reclaims their seat. */
const roomPidKey = (code: string) => `sudoku:online:pid:${code}`;
...
export const useOnlineStore = create<OnlineStore>((set, get) => {
  ...
  const broadcastState = () => {
    ...
    useGameStore.getState().applyRemoteState(hostState, get().mySlot ?? 0);
  };
  ...
  const buildGame = (): GameState => {
    ...
    return createGame({
      ...
      online: true,
      // The authoritative copy has no single owner; each device swaps in its
      // own seat when the snapshot lands (see `applyRemoteState`).
      mySlot: 0,
      players,
    });
  };
  ...
  const guestHandler = (msg: NetMessage) => {
    if (msg.t === 'lobby') {
      ...
      lastSeq = msg.seq;
      useGameStore.getState().applyRemoteState(msg.state, get().mySlot ?? 0);
  ...
  return {
    ...
    sendEmoji: (emoji) => {
      useReactionsStore.getState().show(emoji, true); // show ours immediately
