...
/**
 * Online lobby: create a room (host) or join by code (guest). The host also
 * picks the match config (board size, difficulty, turn mode). Navigates to the
 * board once the authoritative game begins.
 */
export function OnlineLobbyScreen() {
  ...
  // The host starts the match — show the start rewarded ad first (only the host
  // reaches this; guests never call it), then broadcast the game start.
  ...
  const create = () => {
    ...
    setOnlineName(clean);
    void hostRoom(clean || t('player'), {
      size: setup.size,
      difficulty: setup.difficulty,
      mineDensity: setup.mineDensity,
      turnMode: setup.turnMode,
    });
  };
  const join = () => {
    const clean = name.trim();
  ...
  useEffect(() => {
    if (status === 'playing') router.replace('/minesweeper/game');
  }, [status, router]);
  ...
  if (status === 'connecting') {
    return (
      <SafeAreaView
        style={[styles.screen, styles.centered, { backgroundColor: theme.background }]}
      >
  ...
  // In a room (host or guest): show the code, roster and (host) config.
        ...
        {isHost ? (
          <View style={styles.config}>
            ...
            <View style={styles.mineBlock}>
              <View style={styles.mineHeader}>
                ...
                <Text style={[styles.mineValue, { color: theme.textPrimary }]}>
                  💣 {minesForSize(lobby.config.size, lobby.config.mineDensity)}
                  <Text style={{ color: theme.textSecondary }}>
                    {`   ×${lobby.config.mineDensity.toFixed(1)}`}
                  </Text>
                </Text>
              </View>
              <Slider
                ...
                value={lobby.config.mineDensity}
                onChange={(mineDensity) => setConfig({ mineDensity })}
                theme={theme}
              />
            </View>
            ...
            <Segmented<TurnMode>
              ...
              value={lobby.config.turnMode}
              onChange={(turnMode) => setConfig({ turnMode })}
              hint={
                lobby.config.turnMode === 'sequential'
                  ? t('sequential_hint')
                  : t('parallel_hint')
              }
            />
...
const styles = StyleSheet.create({
  ...
  mineValue: { fontSize: 16, fontWeight: '800' },
  primaryButton: {
